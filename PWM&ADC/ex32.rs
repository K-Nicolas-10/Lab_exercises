...
async fn dim_task(
    ...
    loop {
        ...
        if val <= ONE_THIRD {
            ...
            r.set_duty_cycle_fraction(ONE_THIRD - x, ONE_THIRD);
            g.set_duty_cycle_fraction(x, ONE_THIRD);
            b.set_duty_cycle(0);
        } else if val <= TWO_THIRD {
            let x = val - ONE_THIRD;

            r.set_duty_cycle(0);
            g.set_duty_cycle_fraction(ONE_THIRD - x, ONE_THIRD);
            b.set_duty_cycle_fraction(x, ONE_THIRD);
        } else {
            let x = (val - TWO_THIRD).min(ONE_THIRD);

            r.set_duty_cycle_fraction(x, ONE_THIRD);
            g.set_duty_cycle(0);
            b.set_duty_cycle_fraction(ONE_THIRD - x, ONE_THIRD);
        }
...
async fn main(spawner: Spawner) {
    ...
    spawner
        .spawn(dim_task(r, g, b, adc_pin.degrade_adc(), adc))
        .unwrap();
}

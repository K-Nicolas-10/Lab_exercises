...
async fn button_event(mut s1: ExtiInput<'static>) {
    loop {
...
async fn dim_task(
    ...
    loop {
        ...
        match switch {
            State::On => {
                let lv = adc.blocking_read(&mut adc_pin);
                led.set_duty_cycle_fraction(lv, MAX_VALUE);
            }
            State::Off => {
...
async fn main(spawner: Spawner) {
    ...
    spawner.spawn(button_event(s1)).unwrap();
    spawner
        .spawn(dim_task(r, adc_pin.degrade_adc(), adc))
        .unwrap();
}

//only one led instead of all 4 
